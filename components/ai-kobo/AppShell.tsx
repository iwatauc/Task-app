  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-950 dark:bg-stone-950 dark:text-stone-50">
      ...
      {(!isSupabaseConfigured || !isOpenAiConfigured) && (
        <div className="border-b border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-100">
          <div className="mx-auto max-w-7xl">
            {!isSupabaseConfigured && <p>Supabase 環境変数が未設定です。画面確認はできますが、ログイン・保存には `.env.local` または Vercel 環境変数の設定が必要です。</p>}
            {isSupabaseConfigured && !isOpenAiConfigured && <p>OpenAI API key が未設定です。ログインやライブラリ確認はできますが、画像生成・編集には `OPENAI_API_KEY` が必要です。</p>}
          </div>
        </div>
      )}
