...
1. GitHub にこのリポジトリを push します。すでに GitHub 上にある場合はこの手順は不要です。
2. スマホのブラウザで Supabase にログインし、新規 Project を作ります。
3. Supabase の SQL Editor を開き、`supabase/schema.sql` を全文貼り付けて実行します。
4. Supabase の Project Settings で `Project URL` と `anon public key` を控えます。
5. OpenAI の API key を控えます。
6. スマホのブラウザで Vercel にログインし、GitHub リポジトリを Import します。Framework は Next.js の自動検出で問題ありません。
7. Vercel の Environment Variables に `.env.example` と同じキーを登録します。本番 URL が決まったら `NEXT_PUBLIC_SITE_URL` は Vercel の URL にします。
8. Vercel で Deploy します。完了後に表示される URL をスマホで開けば利用できます。
9. Supabase Auth の URL Configuration に Vercel の本番 URL と `https://your-vercel-domain.vercel.app/auth/callback` を追加します。
10. アプリの `/login` からメールリンクでログインし、`/generate` で生成、`/library` で確認、詳細画面から赤ペイント修正を実行します。

スマホだけで「コード編集まで」やる場合は GitHub Web Editor / github.dev / GitHub Codespaces を使えます。ただし、この MVP を単に開いて使うだけなら、Vercel に一度デプロイするのが一番簡単です。

## 環境変数なしでも画面確認

このリポジトリは Supabase / OpenAI の環境変数が未設定でも `npm run dev` または `npm run build && npm run start` でトップ、生成フォーム、ライブラリ、ログイン画面を開けます。未設定時はモック Supabase クライアントに切り替わるため、ログイン・保存・生成・編集は実行されず、画面上に設定不足のバナーが出ます。実データ保存と OpenAI 生成を使う場合は、下のセットアップ手順で `.env.local` または Vercel 環境変数を設定してください。
